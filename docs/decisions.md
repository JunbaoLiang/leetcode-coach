...
- 2026-07-16 · 聊天健壮性(线上 529 实测反馈):Anthropic client max_retries 提到 4;过载/限流报错文案友好化;发送失败时把用户输入放回输入框不丢字;所有 Enter 发送的输入框加 IME 组合态保护(isComposing / keyCode 229),拼音选词回车不再误发送。
- 2026-07-16 · 今日计划从"每次请求现算"改为**每日快照**(新表 daily_plans,每用户每 UTC 日一份冻结清单):原设计做完一题立刻补位,清单永不变短、进度不可见,是反激励(dogfooding 反馈)。完成状态由当日 finished attempts 推导(不存 done 标志,永不失同步);计划外做的题自动以"加餐 ✓"呈现;「再来一道(加餐)」在计划全部完成后可用,一次一道,不计入计划分母——保护"完成计划"的成就感,同时奖励超额。
